...
URL: https://docs.drift.trade/developers/drift-sdk/builder-codes
...
Create separate `DriftClient` instances for the builder and user authorities, each subscribed.

```typescript
const builderClient = new DriftClient({ connection, wallet: builderWallet, env: "mainnet-beta" });
await builderClient.subscribe();

const userClient = new DriftClient({ connection, wallet: userWallet, env: "mainnet-beta" });
