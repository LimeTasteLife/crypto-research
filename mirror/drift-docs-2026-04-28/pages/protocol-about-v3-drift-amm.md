...
Drift uses a backstop AMM as a source of guaranteed constant liquidity, providing constant liquidity to be taken against asynchronously.
...
You can read more about Drift's v1 AMM design here.

## Drift v3 AMM

Drift's v3 AMM is still a AMM using a constant product curve but now includes external Backstop AMM Liquidity (BAL) a concentration factor, and dynamic spread/peg that programmatically update prior to filling trades.
...
To implicitly utilise a dynamic fee, Drift v3's AMM quotes different prices for buys and sells. The offset from the mid-price is dynamic based on the current inventory.
