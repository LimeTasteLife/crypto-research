---
type: entity
subtype: programme
name: dYdX Surge Program
status: live
sources_count: 1
last_reviewed: 2026-04-29
disputed: false
---

# dYdX Surge Program

> Consolidated rewards program replacing protocol-level trading rewards; protocol-level Rewards Module C constant set to 0.

## Snapshot
- Community aligned on removing protocol-level trading rewards; all rewards consolidated under Surge Program[^dydx-community-2026-04-28-modules-trading-rewards]
- Protocol-level C constant set to 0 (formula exists in code but is dormant)[^dydx-docs-2026-04-28-concepts-trading-rewards-trading-rewards]

## Related
[[entities/perpdex/dydx]] · [[parameters/dydx-v4/trading-rewards-config]]

## Sources
[^dydx-community-2026-04-28-modules-trading-rewards]: [[sources/dydx-community-2026-04-28-modules-trading-rewards]]
[^dydx-docs-2026-04-28-concepts-trading-rewards-trading-rewards]: [[sources/dydx-docs-2026-04-28-concepts-trading-rewards-trading-rewards]]
