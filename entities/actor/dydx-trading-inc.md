---
type: entity
subtype: actor
name: dYdX Trading Inc.
status: live
sources_count: 2
last_reviewed: 2026-04-29
disputed: false
---

# dYdX Trading Inc.

> The company that open-sourced the dYdX v4 software; does NOT run any of the protocol, Indexer, or front-end components.

## Snapshot
- Open-sourced v4 node software, Indexer, and front-end code[^dydx-docs-2026-04-28-concepts-architecture-overview]
- Does not operate any of the protocol, Indexer, or front-end[^dydx-docs-2026-04-28-concepts-architecture-overview]
- Investor transfer restrictions enforced via contractual agreements with dYdX Foundation and dYdX Trading Inc.[^dydx-community-2026-04-28-start-here-dydx-token-allocation]

## Related
[[entities/perpdex/dydx]] · [[entities/actor/dydx-foundation]]

## Sources
[^dydx-docs-2026-04-28-concepts-architecture-overview]: [[sources/dydx-docs-2026-04-28-concepts-architecture-overview]]
[^dydx-community-2026-04-28-start-here-dydx-token-allocation]: [[sources/dydx-community-2026-04-28-start-here-dydx-token-allocation]]
